...
public class UnitDefGsonTypeAdapter implements JsonSerializer<UnitDef>, JsonDeserializer<UnitDef> {

	@Override
	public UnitDef deserialize(JsonElement json, Type arg1, JsonDeserializationContext context) throws JsonParseException {
		Gson gson = new GsonBuilder().create();
		JsonObject obj = json.getAsJsonObject();

		// Retrieve everything except non-primitive types
		JsonElement params = obj.get(Constant.levelNormalParamsTag);
		UnitDef temp = gson.fromJson(params, UnitDef.class);
		...
		return new UnitDef(temp.collisionID, temp.name, temp.jGameGraphicPath, temp.team, temp.x, temp.y, 
				temp.row, temp.col, temp.stats, temp.growths, temp.type, temp.originalGraphicPath, inv, weapons, actions, temp.objName);
	}

	@Override
	public JsonElement serialize(UnitDef udef, Type type, JsonSerializationContext context) {
		Gson gson = new Gson();
		JsonObject jsonObj = new JsonObject();
		
		// add primitive types
		jsonObj.add(Constant.levelNormalParamsTag, gson.toJsonTree(udef, UnitDef.class));
