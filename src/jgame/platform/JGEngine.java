...
public abstract class JGEngine extends Applet implements JGEngineInterface {
	...
	public String defineImageWithScale(String imgName, String tileName, int collisionid, String imagePath, int width, int height) {
		JGImage scaledImg = el.imageutil.loadImage(imagePath).scale(width, height);
		String imageName = imgName + System.currentTimeMillis();
		el.defineImage(imageName, tileName, collisionid, scaledImg, "-", 0, 0, -1, -1);
		return imageName;
	}
	
	public String defineImageWithScale(String imgName, String tileName, int collisionid, int width, int height) {
		JGImage scaledImg = getImage(imgName).scale(width, height);
		String imageName = imgName + System.currentTimeMillis();
		el.defineImage(imageName, tileName, collisionid, scaledImg, "-", 0, 0, -1, -1);
		return imageName;
	}
	...
	public boolean existsImage(String imageName) {
		return el.existsImage(imageName);
	}
