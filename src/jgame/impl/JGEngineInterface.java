...
*/
	...
	*/
	...
	/** Define new sprite/tile image from a data array.  If an image with this 
	* id is already defined, it is removed from any caches, so that the old
	* image is really unloaded.  This can be used to load large (background)
	* images on demand, rather than have them all in memory.  Note that the
	...
	* some platforms support parallax scrolling.  On Android, a call to
	* this method will activate parallax scrolling mode, which is less
	* efficient that basic mode.  Parallax mode is disabled again by setting
	* all BG images to null.  On other platforms, only the
	...
	*/
	 ...
	 * Only supported by parallax scrolling platforms.  On Android, a call to
	* this method will activate parallax scrolling mode, which is less
	* efficient that basic mode.  Parallax mode is disabled again by setting
	* all BG images to null.  Note that parallax
	...
	* @param nr  1=button 1 ... 3 = button 3 */
	...
	/** Returns true if multitouch supported (currently only on Android
	 * devices). */
	public boolean hasMultitouch();
	/** get IDs of touch pointers for multitouch handling.  You can use
	 * getMouse... instead, if multitouch is not required.
	 * <p>
	 * Touch IDs are globally unique numbers identifying multitouch finger
	 * trajectories (touch-drag-release).
	 * The IDs returned by this function can be used to get all the info you
	 * need for tracking finger touches and releases.  The list is refreshed
	 * every frame. Use getTouchX/Y to get info about the positions.
	 * @return list of IDs, or null when multitouch not supported. */
	public int [] touchGetIDs();
	/** get X position of given touch pointer. Returns 0 when ID does not
	 * exist. */
	public double touchGetX(int id);
	/** get Y position of given touch pointer. Returns 0 when ID does not
	 * exist. */
	public double touchGetY(int id);
	/** Check if touch ID is still active.  This is equivalent to calling
	 * touchGetIDs and searching the result for the given ID.
